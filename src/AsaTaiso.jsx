...
export default function AsaTaiso() {
  ...
  // ---------- タイマー画面（連続コマ表示） ----------
  function TimerSession({ items, onFinish, onQuit, label, confirmQuit }) {
    ...
    return (
      <div className="flex flex-col items-center px-5 pt-5 pb-8 min-h-full">
        ...
        {askQuit ? (
          <div className="mt-auto pt-6 w-full max-w-xs text-center">
            ...
            <div className="flex gap-3">
              <button onClick={onQuit}
                ...
                style={{ background: "#D9736B" }}>
                やめる
              </button>
              <button onClick={() => { setAskQuit(false); setPaused(false); }}
          ...
          <div className="flex gap-3 mt-auto pt-6 w-full max-w-xs">
            ...
            <button onClick={() => { if (confirmQuit) { setPaused(true); setAskQuit(true); } else { onQuit(); } }}
              className="px-5 py-3 rounded-full text-sm" style={{ color: C.inkSoft }}>
              やめる
            </button>
          </div>
        )}
  ...
  function Home() {
    ...
    return (
      <div className="flex flex-col px-6 pt-8 pb-6 min-h-full">
        ...
        <button onClick={resetAll} className="mt-auto pt-6 text-xs self-center font-bold"
          style={{ color: resetArm ? "#D9736B" : "rgba(122,140,160,0.6)" }}>
          {resetArm ? "もう一度タップで全記録を削除" : "記録をリセット"}
        </button>
      </div>
    );
  }
    ...
    <div className="w-full min-h-screen flex justify-center"
      ...
      <div className="w-full max-w-md flex flex-col">
        {loading ? (
          ...
          <TimerSession items={MOVEMENTS} label="ラジオ体操" confirmQuit
